...
SKXKeys translate_keycode(u32 x_keycode) {
  switch (x_keycode) {
  case XK_BackSpace:
...
b8 SKXPlatformState::startup(const char *application_name, i32 x, i32 y,
  ...
  if (xcb_connection_has_error(state->connection)) {
    SKX_FATAL("Failed to connect X server via XCB");
    return FALSE;
  }
  ...
  // Tell the server to notify when the window mananger
  ...
  if (stream_result <= 0) {
    SKX_FATAL("An error occurred when flusing the stream: %d", stream_result);
    return FALSE;
  }
...
b8 SKXPlatformState::pump_messages() {
  ...
  while (event != 0) {
    ...
    case XCB_KEY_RELEASE: {
      // Key press event - xcb_key_press_event_t and xcb_key_release_event_t are
      // the same
      ...
      SKXKeys key = translate_keycode(key_sym);
    ...
    case XCB_MOTION_NOTIFY: {
      ...
      // Pass over to the input subsystem.
      skx_input_process_mouse_move(move_event->event_x, move_event->event_y);
    } break;
...
void skx_platform_free_memory(void *block, b8 aligned) { free(block); }
...
void skx_platform_get_required_extension_names(const char ***names_darray) {
  {
    decltype(&"VK_KHR_xcb_surface") temp = &"VK_KHR_xcb_surface";
    *names_darray = (const char **)_darray_push(*names_darray, &temp);
  };
}
