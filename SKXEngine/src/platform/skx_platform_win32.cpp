...
b8 SKXPlatformState::startup(const char *application_name, i32 x, i32 y,
  ...
  wc.lpszClassName = "SplashKitX Window Class";

  if (!RegisterClassA(&wc)) {
    MessageBoxA(0, "Window class registration failed", "Error",
  ...
  HWND handle =
      CreateWindowExA(window_ex_style, wc.lpszClassName, application_name,
  ...
  if (handle == 0) {
    ...
    SKX_FATAL("Window creation failed!");
    return FALSE;
  } else {
    state->hwnd = handle;
  }
...
void skx_platform_sleep(u64 ms) { Sleep(ms); }
...
void skx_platform_get_required_extension_names(const char ***names_darray) {
  {
    decltype(&"VK_KHR_win32_surface") temp = &"VK_KHR_win32_surface";
